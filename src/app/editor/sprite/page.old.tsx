...
import type { Tool, SimpleShape, SpriteType, SpriteData, ShapeStart, Selection } from './types'
import { createEmptySprite, validateSpriteSize } from './utils/spriteUtils'
...
export default function SpriteEditorPage() {
  ...
  // Sprite dimensions (separate width/height)
  ...
  // Current frame being edited (for frames/animation sprites)
  ...
  // Current sprite pixel data (for static or current frame)
  ...
  // Load sprites from cart - filter out .rpi files (they're 480x270 backgrounds, not sprites)
  useEffect(() => {
    if (cart?.sprites && Object.keys(cart.sprites).length > 0) {
      // Filter out any entries that might be .rpi files or invalid sprites
      const filteredSprites: SpriteMap = {}
      for (const [name, spriteData] of Object.entries(cart.sprites)) {
        // Skip .rpi files (they should never be in sprites.json, but filter just in case)
        if (name.toLowerCase().endsWith('.rpi')) {
          console.warn(`[SpriteEditor] Skipping .rpi file "${name}" from sprites - .rpi files are 480x270 backgrounds and should not be in sprites.json`)
          continue
        }
        // Validate sprite has required fields
        if (spriteData && typeof spriteData === 'object' && 'width' in spriteData && 'height' in spriteData) {
          // Check if it's a valid sprite size (not 480x270 which is .rpi format)
          if (spriteData.width === 480 && spriteData.height === 270) {
            console.warn(`[SpriteEditor] Skipping sprite "${name}" with size 480x270 (this is .rpi format, not a sprite)`)
            continue
          }
          filteredSprites[name] = spriteData
        } else {
          console.warn(`[SpriteEditor] Skipping invalid sprite entry "${name}"`)
        }
      }
  ...
  useEffect(() => {
    if (selectedSpriteName && sprites[selectedSpriteName]) {
      ...
      // Load frames and animations if present
      ...
      // Load pixel data for static sprites or first frame
      if (spriteData.type === 'static' || !spriteData.type) {
        ...
        // Multi-frame sprite - load first frame
        setSprite(spriteData.frames[0].pixels)
        setCurrentFrameName(spriteData.frames[0].name)
      } else {
        setSprite(createEmptySprite(spriteData.width, spriteData.height))
  ...
  const saveCurrentSprite = useCallback(() => {
    ...
    // Validate size
    const sizeError = validateSpriteSize(width, height, isUI)
    if (sizeError) {
      console.error(`[SpriteEditor] Cannot save sprite: ${sizeError}`)
      return
    }
    ...
    if (spriteType === 'static') {
      ...
      // For frames/animation sprites, save frames array
      if (frames.length > 0) {
        spriteData.frames = frames.map(f => ({ ...f }))
      }
      
      // For animation sprites, save animations array
  ...
  const handleAddSprite = () => {
    ...
    setHistory([createEmptySprite(16, 16)])
    setHistoryIndex(0)
  ...
  const handleAddFrame = useCallback(() => {
    ...
    setHistory([newFrame.pixels])
    setHistoryIndex(0)
  }, [selectedSpriteName, frames, width, height])
  ...
  }, [frames, currentFrameName, width, height])
  ...
  // Drawing algorithms - use imported functions with width/height context
  ...
  // All drawing functions now use imported utilities from drawingTools.ts
  ...
  const handlePaste = useCallback((x: number, y: number) => {
    ...
    setSelection(null)
  }, [clipboard, sprite, width, height, saveToHistory])
  ...
  const handleCanvasMouseUp = useCallback(() => {
    ...
    setIsDrawing(false)
  }, [isDrawing, tool, shapeStart, previewSprite, sprite, saveToHistory, isSelecting, selection, width, height])
  ...
  }, [selection, clipboard, width, height, isSelecting, handleCopy, handlePaste])
  ...
  const rotateCW = useCallback(() => {
    ...
    setSprite(newSprite)
    saveToHistory(newSprite)
  }, [sprite, width, height, saveToHistory])

  const rotateCCW = useCallback(() => {
    // Rotate 90° counter-clockwise: swap width/height
    const newSprite: SpriteData = Array(height).fill(null).map(() => Array(width).fill(-1))
    for (let y = 0; y < height; y++) {
      for (let x = 0; x < width; x++) {
    ...
    <div className="h-full flex flex-col bg-gray-950">
        ...
        {selectedSpriteName && (
          <>
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  if (isUI && newWidth % 2 !== 0) {
                    // For UI sprites, ensure divisible by 2
                    setWidth(newWidth - (newWidth % 2))
                  } else {
                    setWidth(newWidth)
                  }
                }}
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  if (isUI && newHeight % 2 !== 0) {
                    // For UI sprites, ensure divisible by 2
                    setHeight(newHeight - (newHeight % 2))
                  } else {
                    setHeight(newHeight)
                  }
                }}
            ...
            <select
              ...
              value={spriteType}
              onChange={(e) => {
                const newType = e.target.value as SpriteType
                setSpriteType(newType)
                // When switching types, initialize appropriately
                if (newType === 'static') {
                  // Convert current sprite to static pixels if needed
                  if (!sprites[selectedSpriteName!]?.pixels) {
                    setSprite(createEmptySprite(width, height))
                  }
                } else if (newType === 'frames' || newType === 'animation') {
                  // If no frames exist, create one from current sprite
                  if (frames.length === 0) {
                    const frameName = `frame_${Date.now()}`
                    const newFrame: SpriteFrame = {
                      name: frameName,
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  if (newIsUI) {
                    ...
                    if (width % 2 !== 0) setWidth(width - (width % 2))
                    if (height % 2 !== 0) setHeight(height - (height % 2))
                    // Cap at 256
                    if (width > 256) setWidth(256)
                    if (height > 256) setHeight(256)
                  } else {
                    // Cap at 32 for gameplay
                    if (width > 32) setWidth(32)
                    if (height > 32) setHeight(32)
                  }
                }}
                  ...
                  <>
                    ...
                    <div className="flex items-center gap-2 text-xs">
                      ...
                      <input
                        ...
                        value={shapeSize}
                        onChange={(e) => setShapeSize(Math.max(1, Math.min(Math.max(width, height), Number(e.target.value))))}
              ...
              <div className="flex-1 overflow-auto p-3">
                <div className="max-w-[1600px] mx-auto grid grid-cols-1 lg:grid-cols-[280px_1fr] xl:grid-cols-[300px_1fr_280px] gap-4">
                  ...
                  <div className="lg:col-span-1 space-y-4">
                        {/* Frame List (for frames/animation sprites) */}
                        ...
                        {/* Animation Editor (for animation sprites) */}
                        ...
                        {/* Animation Preview (for animation sprites) */}
                        {spriteType === 'animation' && selectedAnimation && (
                          <AnimationPreview
                            ...
                            onSetSpeed={(speed) => handleUpdateAnimation(selectedAnimation.name, { speed })}
                            onSetFrame={(frameIndex) => {
                              // Update preview frame index
                            }}
                          />
                        )}
      ...
      {showSpriteSheetImport && (
        <SpriteSheetImport
          ...
          spriteHeight={height}
          currentPalette={currentPalette}
          onImportFrames={handleImportSpriteSheet}
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-sm text-gray-300 mb-2">
                  Size (1:1, max {Math.max(width, height)})
                </label>
                <input
                  type="range"
                  min="1"
                ...
                <div className="flex justify-between text-xs text-gray-400 mt-1">
                  ...
                  <span>{Math.max(width, height)}</span>
                </div>
              </div>
...
// Utility functions moved to utils/spriteUtils.ts
